...
def write_tb_text(writer, args, step = 0):
    ...
    for attr in args_to_write:
        if attr == "lr_scheduler":
            if getattr(args, attr):
                text = "type: {}, gamma: {} , step: {}".format(getattr(args, attr), getattr(args, "gamma"), getattr(args, "step"))
            else:
                text = "None"
...
class modified_Image_Folder(datasets.DatasetFolder):
    """docstring for modified_Image_Folder"""
    def __init__(self, root, transform=None, target_transform=None,
                 loader=default_loader, is_valid_file=None, stage = None):
        super(modified_Image_Folder, self).__init__(root, loader, IMG_EXTENSIONS if is_valid_file is None else None,
                                          transform=transform,
                                          target_transform=target_transform,
                                          is_valid_file=is_valid_file)

        self.error_file = []
        print("new_image_Folder")
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            pd.DataFrame(self.error_file).to_csv("error_files_{}.csv".format(self.stage))
...
def main():
    ...
    if "b3" in name:
        setattr(args, "lr", args.lr*2)
        setattr(args, "batch_size", args.batch_size//2)
    elif "b7" in name:
        setattr(args, "lr", args.lr*4)
        setattr(args, "batch_size", args.batch_size//4)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.no_augment:
        print("no_augment")
        train_dataset = modified_Image_Folder(
        traindir,
        transforms.Compose([
    ...
    val_loader = torch.utils.data.DataLoader(
        modified_Image_Folder(valdir, transforms.Compose([
...
def save_checkpoint(state, is_best, name = "", filename='checkpoint.pth.tar'):
    ...
    torch.save(state, "weights/{}".format(new_name))
    if is_best:
        shutil.copyfile("weights/{}".format(new_name), "weights/{}".format(name + '_model_best.pth.tar'))
